...
bool RandoSession::extractFile(CacheEntry& current)
{
...
bool RandoSession::repackFile(CacheEntry& current)
{
    GenericFile* parentData = dynamic_cast<GenericFile*>(current.parent->data.get());
    ...
    switch(current.storedFormat) {
        ...
        case Fmt::STREAM:
        {
            if (current.parent->storedFormat == Fmt::SARC) {
                ...
                file->replaceEmbeddedFile(current.element.string(), dynamic_cast<GenericFile*>(current.data.get())->data.str());
            }
        }
        return true;
        case Fmt::ROOT:
...
bool RandoSession::handleChildren(CacheEntry& current) {
    ...
    //repack this level once we are done
    if(!repackFile(current)) return false;
...
bool RandoSession::modFiles_singleThread()
{
    CHECK_INITIALIZED(false);
    
    while(fileCache->children.size() != 0) {
        for(auto it = fileCache->children.begin(); it != fileCache->children.end(); ) {
            std::shared_ptr<CacheEntry> child = it->second;
            bool reqsDone = true;
            ...
            if(!handleChildren(*child)) return false;
            it = fileCache->children.erase(it);
        }
    }
